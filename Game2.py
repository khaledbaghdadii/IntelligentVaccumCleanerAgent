...
class Game2:
    def __init__(self,n,m):
        ...
        self.random_dist_list=DropDown(250, 550, 130, 30,  "Randomness %", ["Uniform: 50%", "Uniform: 12.5%"])
        ...
        self.recharge_btn= Button("Recharge All",(400,650),font=25,bg=(200,150,50))
        self.recharge_value=100
        self.dropdown=DropDown(530, 550, 150, 30,  "Select Speed", ["Very Slow", "Slow","Medium","Fast","Very Fast"])
        ...
        self.performanceLabel=TextLabel('Cleaner',750,380,font_background=(255,255,255),font=pygame.font.SysFont(None, 25))
        self.multiCleaners=DropDown(810, 365, 80, 30,  "1", self.vacuum_cleaners_indices)
        self.moves_label=TextLabel('Number of Moves: 0',1040,190,font_background=(255,255,255),font=pygame.font.SysFont(None, 25))
        self.explored_label=TextLabel('Number of Explored: 0 ',1050,220,font_background=(255,255,255),font=pygame.font.SysFont(None, 25))
        ...
        self.battery=TextLabel('Battery%: 0 ',770,420,font_background=(255,255,255),font=pygame.font.SysFont(None, 25))


        self.performanceLabel2=TextLabel('Dirt Agent',1020,380,font_background=(255,255,255),font=pygame.font.SysFont(None, 25))
        self.multiDirtiers=DropDown(1100, 365, 80, 30,  "0", self.dirt_agents_indices)
        ...
        self.battery2=TextLabel('Battery%: 0 ',1030,420,font_background=(255,255,255),font=pygame.font.SysFont(None, 25))
        #Dirt Frequencies
        self.dirtFrequency=TextLabel('Dirts Freq.:',1030,455,font_background=(255,255,255),font=pygame.font.SysFont(None, 25))
        self.dirts_freq_input=InputBox(1100,440,80,30)
        self.recharge_value_input=InputBox(1100,520,80,30)
        self.rechargeValueText=TextLabel('Recharge Value:',1030,535,font_background=(255,255,255),font=pygame.font.SysFont(None, 25))

        self.scores=TextLabel('Scores',750,570,font_background=(255,255,255))
        self.winnerCleanerAgent=TextLabel('Winner Cleaner Agent:         ',845,600,font_background=(255,255,255),font=pygame.font.SysFont(None, 25))
        self.WinnerDirtAgent=TextLabel('Winner Dirt Agent:        ',825,630,font_background=(255,255,255),font=pygame.font.SysFont(None, 25))
        self.BothAgent=TextLabel('Overall Winner:                           ',860,660,font_background=(255,255,255),font=pygame.font.SysFont(None, 25))
        ...
        self.cleaning_agents_battery.append(self.recharge_value)
    ...
    def draw(self):
        ...
        self.dirtFrequency.draw(self.screen)
        self.rechargeValueText.draw(self.screen)
        self.dirts_freq_input.draw(self.screen)
        self.recharge_value_input.draw(self.screen)
    ...
    def handle_events(self):
        ...
        for event in pygame.event.get():
            ...
            self.recharge_value_input.handle_event(event)
            ...
            if event.type ==pygame.MOUSEBUTTONDOWN :
                ...
                if pygame.mouse.get_pressed()[0]:
                    ...
                    if(self.add_dirt_agent_checkbox.checked and y1<=self.m-1 and x1<=self.n-1 and ((x1,y1) not in self.added_agents_on_gui)):
                        ...
                        self.dirt_agents_battery.append(self.recharge_value)
                        ...
                        if self.recharge_value_input.text=="" or int(self.recharge_value_input.text)<0:

                            pass
                        else:
                            self.recharge_value=int(self.recharge_value_input.text)
    ...
    def resetGrid(self):
            ...
            self.battery.setText('Battery%: ' +str(self.recharge_value))
    ...
    def startAllAgentsMiniMax(self):
        for i in range(len(self.DirtAgents)):
             ...
             if ((dirtAgentTile.x,dirtAgentTile.y) not in self.Dirts.dirts_array and self.DirtAgents[i].count%self.dirts_freq_used==0 and self.DirtAgents[i].battery>=10):
        ...
        for i in range(len(self.VacuumCleaners)):
             ...
             if self.Tiles.tiles[cleaningAgentTile.x][cleaningAgentTile.y].isDirty:
                ...
                if ((cleaningAgentTile.x,cleaningAgentTile.y) in self.Dirts.dirts_array and self.VacuumCleaners[i].battery>=10):
    ...
    def startAllDirtAgentsRandom(self):
        ...
        if self.multialgo_dropdown.main=="Minimax - Random":
            for i in range(len(self.VacuumCleaners)):
                ...
                if(self.VacuumCleaners[i].battery>=5):
                    self.VacuumCleaners[i].move(tile.x-self.VacuumCleaners[i].x,tile.y-self.VacuumCleaners[i].y)
                    self.VacuumCleaners[i].battery-=5
                    sleep(self.WAIT_TIME)
                self.draw()
                pygame.event.pump()
        else:        
    ...
    def getEvaluationScores(self):
        ...
        if winner_type == 0:
            ...
            self.BothAgent.setText("Overall Winner - Cleaner: "+str(" (")+str(self.VacuumCleaners[overall_winner_index].x)+str(",") +str(self.VacuumCleaners[overall_winner_index].y)+str(")"))

        else:
            self.BothAgent.setText("Overall Winner - Dirt Agent: "+str(" (")+str(self.DirtAgents[overall_winner_index].x)+str(",") +str(self.DirtAgents[overall_winner_index].y)+str(")"))
...
#project one stuff
    ...
    def startDirtAgentRandom(self,i):
        ...
        if(self.counts[i]%self.dirts_freq_used==0 and (dirt_agent.x,dirt_agent.y) not in self.Dirts.dirts and self.DirtAgents[i].battery>=10):
